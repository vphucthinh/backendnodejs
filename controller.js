...
export const fetchCartsByID = async (data) => {
...
export default {
    ...
    saveCarts: async (req, res, next) => {
        try {
            const listCart = req.body;
            // console.log(listCart?.listcart);

            let rs = []
            listCart?.listcart?.forEach(async element => {

                await DB.execute(
                    'INSERT INTO `carts` (`name`,`qty`,`itemId`,`price`, `uid`,`imgSrc`) VALUES (?,?,?,?,?,?)',
                    [element?.name, element?.qty, element?.id, element?.price, element?.uid, element?.imgSrc]
                );
            })
    ...
    getCarts: async (req, res, next) => {
        try {
            ...
            const listMyCarts = await fetchCartsByID(data.id);
            res.json({
                status: 200,
                listMyCarts: listMyCarts,
            });
        } catch (err) {
            next(err);
        }
    ...
    getUser: async (req, res, next) => {
        try {
            ...
            res.json({
                status: 200,
                user: { ...user[0], username: user[0].name },
            });
        } catch (err) {
            next(err);
        }
